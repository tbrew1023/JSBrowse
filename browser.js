var browser = (function(configModule, tabsModule) {
  ...
  var Browser = function(
    ...
    this.permissionBoxController = new permissionTool.PermissionController(permissionbox, this);
...
*/
  ...
  Browser.prototype.init = function() {
    (function(browser) {
      ...
      // ---------- NAVIGATION / RELOAD EVENTS ----------

      browser.back.addEventListener('click', function(e) {
        browser.tabs.getSelected().goBack(); //def in tabs class
      });

      browser.forward.addEventListener('click', function() {
        browser.tabs.getSelected().goForward(); //def in tabs class
      });
      ...
      // ---------- URL LOCATION BAR EVENTS ----------

      browser.locationForm.addEventListener('submit', function(e) {
        e.preventDefault();
        browser.closeAllMessagesAndDialoges();

        var locationBarInput = browser.locationBar.value;

        var contextHTTP = (locationBarInput.substring(0,7) == 'http://' || locationBarInput.substring(0,8) == 'https://' ? true : false);
        var contextLocalhost = (locationBarInput.substring(0,9) == 'localhost' ? true : false);
        //var contextSearch = (!contextHTTP && !contextHTTPS && (locationBarInput.substring(0,9) != 'localhost') ? true : false);

        //stage webdesk custom commands

        var url;

        if(contextHTTP) {
          url = locationBarInput; //no additional suffix
        }
        else {
          url = 'http://' + locationBarInput; //http suffix on localhost
        ...
        browser.tabs.getSelected().navigateTo(url); //def in tabs class
      });
  ...
  */
  ...
  Browser.prototype.doKeyDown = function(e) {
    ...
    if (e.ctrlKey) {
      ...
      // Ctrl + [1-9]   =>   Switch between tabs w keyboard shortcuts
