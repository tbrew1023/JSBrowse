var tabs = (function(popupModule, contextMenuModule) {
  ...
  */
  ...
  var TabList = function(name, browser, tabContainer, contentContainer, newTabElement) { //constructor
    this.name = name;
    this.list = [];
    this.table = {};
    this.selected = 0;
  ...
  TabList.prototype.selectTab = function(tab, idx) {
    ...
    tab.select();
    this.browser.doTabSwitch(prevTab, tab); //def in browser.js
  ...
  TabList.prototype.removeTab = function(tab, idx) {
    if (this.list.length > 1) {
      ...
      this.browser.closeBrowser(); // close browser if only 1 tab is left ( return to webDesk home )
      return null;
    }
  };
